...
--divideBy10 n = n `div` 10
	     
-- howManyZeroes :: Int -> Int
...
--toDigits n
--	  | n < 0 = error "Numero negativo nao eh aceito como parametro!"
--	  | n == 0 = [0]
--	  | otherwise = (x : xs)
--	  			where
--	  			      x = read (head (show n)) :: Int
--	  			      xs = read (drop 1 (show n)) :: Int
--toDigits n
--	   | n < 0 = error "fucking error!"
--	   | n == 0 = []
--   	   | otherwise = (n `div` (10 ^ (k - 1))) : (toDigits numero)
--	   | otherwise = ((n - numero) `div` (10 ^ (k - 1))) : (toDigits numero)
--			where
--				k = (length (createList n))
--				numero = n `mod` 10 ^ (k - 1)
